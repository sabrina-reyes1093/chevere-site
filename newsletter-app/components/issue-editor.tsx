...
const emptyRecommendation = (): Recommendation => ({ title: "", text: "", url: "" });
  ...
  return <>
    ...
    <div className="editor-layout">
      <form className="editor stack" onSubmit={(event) => { event.preventDefault(); void save(); }}>
        ...
        <fieldset disabled={busy || locked}><legend>This Week’s Edit</legend>{issue.recommendations.map((item, index) => <div className="recommendation" key={index}><strong>Recommendation {index + 1}</strong><input aria-label={`Recommendation ${index + 1} title`} placeholder="Title" value={item.title} onChange={(e) => recommendation(index, "title", e.target.value)} /><textarea aria-label={`Recommendation ${index + 1} description`} rows={3} placeholder="Why it’s worth discovering" value={item.text} onChange={(e) => recommendation(index, "text", e.target.value)} /><input aria-label={`Recommendation ${index + 1} link`} type="url" placeholder="https://" value={item.url} onChange={(e) => recommendation(index, "url", e.target.value)} /></div>)}{issue.recommendations.length < 5 && <button type="button" className="secondary" onClick={() => setIssue((current) => ({ ...current, recommendations: [...current.recommendations, emptyRecommendation()] }))}>Add recommendation</button>}</fieldset>
        <fieldset disabled={busy || locked}><legend>The Chévere Read</legend><label>Select a published article<select value="" onChange={(e) => chooseArticle(e.target.value)}><option value="">Choose from the website…</option>{articles.map((article) => <option key={article.url} value={article.url}>{article.title}</option>)}</select></label><label>Article title<input value={issue.featured_title} onChange={(e) => field("featured_title", e.target.value)} /></label><label>Preview<textarea rows={4} value={issue.featured_preview} onChange={(e) => field("featured_preview", e.target.value)} /></label><div className="two-col"><label>Article link<input type="url" value={issue.featured_url} onChange={(e) => field("featured_url", e.target.value)} /></label><label>Image URL<input type="url" value={issue.featured_image_url} onChange={(e) => field("featured_image_url", e.target.value)} /></label></div></fieldset>
