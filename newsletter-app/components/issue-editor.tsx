  ...
  return <>
    ...
    <div className="editor-layout">
      <form className="editor stack" onSubmit={(event) => { event.preventDefault(); void save(); }}>
        ...
        <fieldset disabled={busy || locked}><legend>This Week’s Edit</legend>{issue.recommendations.map((item, index) => <div className="recommendation" key={index}><strong>Recommendation {index + 1}</strong><input aria-label={`Recommendation ${index + 1} title`} placeholder="Title" value={item.title} onChange={(e) => recommendation(index, "title", e.target.value)} /><textarea aria-label={`Recommendation ${index + 1} description`} rows={3} placeholder="Why it’s worth discovering" value={item.text} onChange={(e) => recommendation(index, "text", e.target.value)} /><input aria-label={`Recommendation ${index + 1} link`} type="url" placeholder="https://" value={item.url} onChange={(e) => recommendation(index, "url", e.target.value)} /><input aria-label={`Recommendation ${index + 1} image`} type="url" placeholder="Optional image URL" value={item.image_url || ""} onChange={(e) => recommendation(index, "image_url", e.target.value)} /></div>)}{issue.recommendations.length < 5 && <button type="button" className="secondary" onClick={() => setIssue((current) => ({ ...current, recommendations: [...current.recommendations, emptyRecommendation()] }))}>Add recommendation</button>}</fieldset>
