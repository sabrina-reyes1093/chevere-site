...
Companion Next.js service for Chévere's confirmed subscriber list, weekly issue editor, email rendering, test sends, unsubscribe processing, delivery logs, and Friday scheduling.
...
- **Email:** Resend, using a verified sender domain.
- **Schedule:** Vercel calls the protected endpoint at 13:30, 14:30, and 15:30 UTC each Friday. The endpoint sends only when the current America/Chicago time is inside the 8:30–10:30 Friday send/retry window. The first eligible invocation is 8:30 a.m. in both CST and CDT; later eligible invocations provide safe retry opportunities.
...
4. Create a Resend account, verify `itschevere.com`, and configure SPF/DKIM as Resend instructs.
5. Deploy this directory to Vercel with `newsletter.itschevere.com` as its custom domain.
6. Add every variable from `.env.example` to Vercel. Use long random values for `TOKEN_SECRET` and `CRON_SECRET`.
...
- A public signup creates one pending subscriber and sends a confirmation email.
- Repeated signup attempts do not create duplicate addresses.
- Confirmation changes the subscriber to active and confirmed.
- The footer unsubscribe link and one-click unsubscribe request immediately mark the subscriber unsubscribed.
...
- An approved issue shows the next Friday at 8:30 a.m. America/Chicago.
...
9. Choose **Approve & schedule** only when the final issue is ready. This schedules it for the next Friday at 8:30 a.m. America/Chicago.

Drafts, empty issues, unapproved issues, previously sent issues, inactive subscribers, unconfirmed subscribers, and unsubscribed addresses are never selected for delivery.
